-- CAMPUS INTEGRATION COMPLETE SQL UPDATE SCRIPT

-- 1. DASHBOARD PROCEDURES
-- ====================================================================================================
...
-- 2. CONSOLIDATED PROCEDURES (Reports, Certificates, Staff, Departments)
-- =========================================================================
...
-- 3. SETTINGS PROCEDURES (Number Series, Templates)
DELIMITER //

DROP PROCEDURE IF EXISTS `sp_GetTemplateForCertificate` //
CREATE PROCEDURE `sp_GetTemplateForCertificate`(
        IN p_ShortCode VARCHAR(20),
        IN p_CodeGuess VARCHAR(50),
        IN p_CanonicalType VARCHAR(100),
        IN p_RawType VARCHAR(100),
        IN p_CampusId INT
)
BEGIN
        ...
        WHERE (`TemplateCode` = p_ShortCode 
               ...
               OR `TemplateCode` = p_RawType)
          AND `IsActive` = 1
          AND (p_CampusId IS NULL OR CampusId = p_CampusId)
        ORDER BY 
            CASE 
                WHEN `TemplateCode` = p_ShortCode THEN 1
                WHEN `TemplateCode` = p_CodeGuess THEN 2
                WHEN `TemplateCode` = p_CanonicalType THEN 3
                WHEN `TemplateCode` = p_RawType THEN 4
                ELSE 5
            END
        LIMIT 1;
END //
...
BEGIN
    SELECT `Id`, `SeriesCode`, `SeriesName`, `Prefix`, `FormatPattern`, 
           `NumberLength`, `StartNumber`, `CurrentSequence`, `Description`, 
           `IsActive`, `CreatedAt`, `UpdatedAt`, `CampusId`
    FROM `NumberSeriesConfigurations`
    WHERE `IsActive` = 1
      AND (p_CampusId IS NULL OR CampusId = p_CampusId)
    ORDER BY `Id` ASC;
...
CREATE PROCEDURE `sp_UpdateNumberSeriesByCode`(
    IN p_SeriesCode VARCHAR(50),
    IN p_CurrentSequence BIGINT,
    IN p_CampusId INT
)
BEGIN
    UPDATE `NumberSeriesConfigurations`
    SET `CurrentSequence` = p_CurrentSequence,
        `UpdatedAt` = UTC_TIMESTAMP()
    WHERE `SeriesCode` = p_SeriesCode
      AND (p_CampusId IS NULL OR CampusId = p_CampusId);
END //
...
BEGIN
    ...
    -- Get current sequence and format
    SELECT `CurrentSequence`, `FormatPattern`, `Prefix`, `NumberLength`
    INTO v_CurrentSequence, v_FormatPattern, v_Prefix, v_NumberLength
    FROM `NumberSeriesConfigurations`
    WHERE `SeriesCode` = p_SeriesCode
      AND (p_CampusId IS NULL OR CampusId = p_CampusId)
    LIMIT 1;

    -- If found, increment and format
...
-- =========================================================
-- SECTION & ROLL NUMBER ALLOCATION (CAMPUS INTEGRATION)
-- =========================================================
...
-- =========================================================
-- EXAMINATION MODULE (CAMPUS INTEGRATION)
-- =========================================================
...
-- =========================================================
-- RESULT MODULE (CAMPUS INTEGRATION)
-- =========================================================
...
-- =========================================================
-- MARKS MODULE (CAMPUS INTEGRATION)
-- =========================================================
