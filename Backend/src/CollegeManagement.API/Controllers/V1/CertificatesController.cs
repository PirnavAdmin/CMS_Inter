...
public class CertificatesController : ControllerBase
{
    ...
    {
        var templates = await _templateService.GetActiveTemplatesByCategoryAsync("Certificate", ct: ct);
        return Ok(templates);
    }
    ...
    public async Task<IActionResult> GetTemplateByCode(string templateCode, CancellationToken ct = default)
    {
        ...
        var template = await _templateService.GetTemplateByCodeAsync(templateCode, ct: ct);
        if (template == null)
        ...
        CancellationToken ct = default)
    {
        try
        {
            var result = await _templateService.RenderTemplateAsync(request, ct: ct);
            return Ok(result);
        }
