...
import { ArrowLeft, BookOpen, ChevronDown, Pencil, Plus, Trash2 } from "lucide-react";
...
export default function SubjectManagementPage() {
  ...
  return <DashboardLayout title="Subject Management" subtitle="Configure and allocate subjects to groups." breadcrumb={["Academics", "Subject Management"]} actions={subjectView === "subjects" ? <button className="cms-btn cms-btn-ghost" type="button" onClick={() => setSubjectView("languages")}>Manage Languages</button> : null}>
    <main className="subject-screen subject-master-screen subject-allocation-screen subject-management-page">
      {subjectView === "languages" ? <>
        <button className="cms-back-link subject-language-back" type="button" onClick={() => setSubjectView("subjects")}><ArrowLeft size={15} /> Back to Subjects</button>
        <section className="subject-table-card subject-language-list"><header className="subject-table-head"><div><h2>Language Subjects</h2><p>Reusable language records for group-wise allocation.</p></div><button className="cms-btn cms-btn-primary" type="button" onClick={() => { setLanguageDraft(emptyLanguageDraft()); setLanguageSelection(""); setLanguageModalOpen(true); }}><Plus size={16} /> Add Language</button></header><div className="subject-table-scroll"><table className="subject-table subject-master-table subject-language-table"><thead><tr><th>Language</th><th>Subject Code</th><th>Total Marks</th><th>Passing Marks</th><th>Action</th></tr></thead><tbody>{languageSubjects.map((subject) => { const scope = /^english$/i.test(displayLanguageName(subject.name)) ? "common" : "secondLanguage"; return <tr key={subjectKey(subject)}><td>{displayLanguageName(subject.name)}</td><td><b>{subject.code}</b></td><td>{total(subject.marks)}</td><td>{markValue(subject.marks?.passing)}</td><td><div className="subject-master-actions"><button className="cms-action-btn" title="Edit language" onClick={() => openConfiguredLanguageEdit(subject, scope)}><Pencil size={16} /></button><button className="cms-action-btn subject-remove-btn" title="Delete language" onClick={() => setRemoving({ ...subject, scope })}><Trash2 size={16} /></button></div></td></tr>; })}</tbody></table></div></section>
      </> : <>
      ...
      <section className="subject-allocation-columns" aria-label="Group subject allocation">
        <section className="subject-table-card subject-allocation-panel">
          ...
          <div className="subject-panel-search app-search-field"><Search3DIcon className="app-search-field__icon" size={17} aria-hidden="true" /><input value={allocationSearch} onChange={(event) => setAllocationSearch(event.target.value)} placeholder={`Search ${selectedGroupName} subjects...`} disabled={!context.groupId} /></div>
        ...
        <section className="subject-table-card subject-allocation-panel">
          ...
          <div className="subject-panel-search app-search-field"><Search3DIcon className="app-search-field__icon" size={17} aria-hidden="true" /><input value={allocatedSearch} onChange={(event) => setAllocatedSearch(event.target.value)} placeholder="Search allocated subjects..." disabled={!context.groupId} /></div>
...
function SubjectCombobox({ options, value, onChange, newName, onNewName, onSearch }) {
  ...
  return <div className="subject-combobox subject-search-wrapper" ref={root}><label className="subject-master-field"><span>{value === "new" ? "Subject *" : "Subject"}</span><span className="app-search-field app-search-field--icon"><input className="subject-search-input" role="combobox" aria-expanded={open && Boolean(filtered.length)} aria-controls="subject-options" aria-autocomplete="list" value={query} placeholder={value === "new" ? "Enter subject name..." : "Search or select subject..."} onFocus={() => filtered.length && setOpen(true)} onChange={type} onKeyDown={keyDown} /></span></label>{open && filtered.length ? <div id="subject-options" className="subject-combobox-options subject-search-results app-select-panel" role="listbox">{filtered.map((item, index) => <button key={item.id} type="button" role="option" aria-selected={item.id === value} className={("subject-search-option " + (index === active ? "is-active" : "")) + " app-select-option"} onMouseDown={(event) => event.preventDefault()} onClick={() => choose(item)}><strong>{item.name}</strong><span>{item.code + " · " + item.type.join(" + ")}</span></button>)}</div> : null}</div>;
}
...
function RequiredLabel({ children }) { return <span>{children}<span className="subject-required-mark" aria-hidden="true">*</span></span>; }
function LanguageForm({ draft, editDraft, editMark, includeName = true }) { return <><div className="subject-form-grid">{includeName ? <Text label={<RequiredLabel>Language</RequiredLabel>} value={draft.name} onChange={(value) => editDraft("name", value)} /> : null}<Text label={<RequiredLabel>Subject Code</RequiredLabel>} value={draft.code} onChange={(value) => editDraft("code", value)} /></div><div className="subject-form-grid subject-language-marks-grid"><MarkNumber label="Theory Marks" value={draft.marks.theory} onChange={(value) => editMark("theory", value)} /><MarkNumber label="Practical Marks" value={draft.marks.practical} onChange={(value) => editMark("practical", value)} /><MarkNumber label="Internal Marks" value={draft.marks.internal} onChange={(value) => editMark("internal", value)} /><Text label="Total Marks" value={total(draft.marks)} type="number" readOnly /></div><MarkNumber label={<RequiredLabel>Passing Marks</RequiredLabel>} value={draft.marks.passing} onChange={(value) => editMark("passing", value)} /></>; }
