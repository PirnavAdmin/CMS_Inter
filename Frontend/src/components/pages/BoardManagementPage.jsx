...
import { Download, Edit3, Eye, Landmark, Plus, Save, Trash2, UsersRound } from "lucide-react";
...
function BoardForm({ mode, value, onCancel, onSave }) {
  ...
  const field = (name, label, options, className = "") => <label className={`board-field ${className}${errors[name] ? " has-error" : ""}`}><span>{label} <b>*</b></span>{options ? <select className="app-select" value={form[name]} onChange={(e) => setValue(name, e.target.value)}><option value="">Select {label}</option>{options.map((option) => <option key={option}>{option}</option>)}</select> : <input value={form[name]} onChange={(e) => setValue(name, e.target.value)} />}{errors[name] ? <small>{errors[name]}</small> : null}</label>;
  return <section className="board-panel board-form-card">
    ...
    <form className="board-form-grid board-form-grid--reference" onSubmit={submit} noValidate>
      ...
      <label className="board-field board-status-field"><span>Status <b>*</b></span><select className="app-select" value={form.status} onChange={(e) => setValue("status", e.target.value)}><option>Active</option><option>Inactive</option></select></label>
...
export default function BoardManagementPage() {
  ...
  return <DashboardLayout title="Board Management" subtitle="Manage education boards and academic settings." breadcrumb={["Academics"]}><main className="board-page board-list-view"><section className="board-panel"><header className="board-list-header"><div><h2>Board History</h2><p>View and manage configured education boards.</p></div></header><div className="board-toolbar board-toolbar--simple"><div className="board-search app-search-field"><Search3DIcon className="app-search-field__icon" size={18} /><input value={search} onChange={(e) => changeSearch(e.target.value)} placeholder="Search board name, code, state..." /></div><div className="board-toolbar-actions"><button className="cms-btn cms-btn-ghost" onClick={exportRows}><Download size={16} /> Export</button><button className="cms-btn cms-btn-primary board-add-button" onClick={() => { setSelected(null); setMode("add"); }}><Plus size={17} /> Add Board</button></div></div>
