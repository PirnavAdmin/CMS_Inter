...
function StudentSummary({ rows }) { const perStudent = rows.map((r) => { const m = studentSessionStatus(r, "morning"), a = studentSessionStatus(r, "afternoon"); if (m === "Present" && a === "Present") return "Present"; if ((m === "Present" && a !== "Present" && a !== "—") || (a === "Present" && m !== "Present" && m !== "—")) return "Half-Day"; if (m === "Present" || a === "Present") return "Present"; if (m === "Absent" || a === "Absent") return "Absent"; return "—"; }).filter((x) => x !== "—"), p = perStudent.filter((x) => x === "Present").length, hd = perStudent.filter((x) => x === "Half-Day").length; return <Summary student data={[["Total Students", rows.length], ["Present", p], ["Absent", perStudent.filter((x) => x === "Absent").length], ["Leave", hd], ["Attendance %", perStudent.length ? `${Math.round((p + 0.5 * hd) * 100 / perStudent.length)}%` : "0%"]]} />; }
...
function StaffSummary({ rows }) {
  return <section className="att-staff-summary">{staffSummaryItems.map(([label, Icon]) => {
    const count = label === "Total Staff" ? rows.length : rows.filter((row) => status(row.status) === label).length;
    const tone = label.toLowerCase().replace(/\s+/g, "-");
    return <article key={label} className={`att-staff-summary-card is-${tone}`}><span className="att-staff-summary-icon"><Icon size={21} strokeWidth={2.1} /></span><span className="att-staff-summary-copy"><small>{label}</small><strong>{count}</strong></span></article>;
  })}</section>;
}
const studentSummaryIcons = { "Total Students": Users, "Total Staff": Users, Present: UserCheck, Absent: UserX, Leave: Coffee, Late: ClipboardClock, "Attendance %": PieChart };
...
function DailyTable({ rows, staff, edit, view, emptyMessage }) { const roll = !staff && rows.some((r) => get(r, "rollNo", "rollNumber") != null), admission = !staff && rows.some((r) => get(r, "admissionNo", "admissionNumber") != null); return <div className="att-scroll"><table className="cms-table att-table"><thead><tr>{staff ? <><th>Staff ID</th><th>Staff Name</th><th>Department</th><th>Designation</th><th>Status</th><th>In Time</th><th>Out Time</th></> : <><th>Student Name</th>{roll ? <th>Roll No</th> : null}{admission ? <th>Admission No</th> : null}<th>Group</th><th>Section</th><th>Morning</th><th>Afternoon</th></>}<th>Action</th></tr></thead><tbody>{rows.length ? rows.map((r, i) => <tr key={get(r, staff ? "facultyId" : "studentId", "id") ?? i}>{staff ? <><td>{get(r, "facultyId", "staffId")}</td><td><button type="button" className="att-name-link" onClick={() => view(r)}>{get(r, "staffName", "facultyName")}</button></td><td>{get(r, "departmentName", "department") || "—"}</td><td>{get(r, "designationName", "staffTypeName") || "—"}</td><td><Pill value={status(r.status)} /></td><td>{get(r, "inTime") || "—"}</td><td>{get(r, "outTime") || "—"}</td></> : <><td><button type="button" className="att-name-link" onClick={() => view(r)}>{get(r, "studentName", "name")}</button></td>{roll ? <td>{get(r, "rollNo", "rollNumber") || "—"}</td> : null}{admission ? <td>{get(r, "admissionNo", "admissionNumber") || "—"}</td> : null}<td>{get(r, "groupName") || "—"}</td><td>{get(r, "sectionName") || "—"}</td><td><Pill value={studentSessionStatus(r, "morning")} /></td><td><Pill value={studentSessionStatus(r, "afternoon")} /></td></>}<td><button className="cms-action-btn" title="Edit Attendance" aria-label="Edit Attendance" onClick={() => edit(r)}><Pencil size={16} /></button></td></tr>) : <tr><td colSpan="9"><div className="cms-empty">{emptyMessage || "No attendance records match the selected filters."}</div></td></tr>}</tbody></table></div>; }
  ...
  return <section className="att-card att-month-card">
    ...
    <div className="att-student-search att-records-search-toolbar att-month-search-toolbar">
      <div className="att-student-search-box">
        ...
        <input type="search" value={search} onChange={(e) => { onSearchChange(e.target.value); onPageChange(1); }} placeholder={staff ? "Search by staff name or staff ID..." : "Search by student name, roll no. or admission no..."} />
      </div>
    </div>
