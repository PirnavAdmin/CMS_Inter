...
export default function SectionAllocationPage() {
  ...
  return <DashboardLayout title={sectionStudents ? "Allocated Students" : "Section & Roll Allocation"} subtitle={sectionStudents ? "Review and update students assigned to the selected section." : "Allocate existing students using backend-validated previews."} breadcrumb={["People", "Section & Roll Allocation"]}>
{sectionStudents ? <main className="allocation-students-screen">
<button type="button" className="cms-back-link allocation-students-back" onClick={() => setSectionStudents(null)}><ArrowLeft size={16} /> Back to Section Allocation</button>
<section className="cms-card allocation-students-card"><CardHead title={get(sectionStudents.section, "sectionName", "SectionName") || "Allocated Students"} text={capacityText({ ...sectionStudents.section, allocatedCount: sectionStudents.allocatedCount, remainingCapacity: sectionStudents.remainingCapacity })} icon={<Users size={20} />} /><StudentTable students={viewedStudentRows} onEdit={openEdit} /></section>
</main> : <>
