...
export default function StudentEnrollmentPage({ id }) {
  ...
  return <DashboardLayout title="Edit Student Profile" subtitle="Update student details and save the changes." breadcrumb={["People", "Students"]} backLinkPosition="below" backLink={<Link className="cms-back-link" to={`/dashboard/students/${id}`} state={returnState ? { studentManagement: returnState } : undefined}><ArrowLeft size={14} /> Back to Student Profile</Link>}><form onSubmit={submit} className="cms-card student-profile-edit" noValidate>
    ...
    <ProfileSection title="Personal Information" className="student-profile-personal-section">
      ...
      <Field label="Gender *" {...field("gender")}><select className="app-select" value={form.gender} onChange={change("gender")}><option value="">Select gender</option>{genderOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select></Field><Field label="Date of Birth *" {...field("dateOfBirth")}><input type="date" value={form.dateOfBirth} onChange={change("dateOfBirth")} /></Field><Field label="Blood Group"><select className="app-select" value={form.bloodGroup} onChange={change("bloodGroup")}><option value="">Select Blood Group</option>{bloodGroups.map((group) => <option key={group} value={group}>{group}</option>)}</select></Field><Field label="Nationality" {...field("nationality")}><input value={form.nationality} onChange={change("nationality")} maxLength="100" /></Field><Field label="Religion"><input value={form.religion} onChange={change("religion")} /></Field><Field label="Category"><select className="app-select" value={form.category} onChange={change("category")}><option value="">Select category</option>{categoryOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select></Field>
...
function SelectField({ label, value, onChange, options, placeholder }) {
  ...
  return <label className="cms-field"><span>{label}</span><select className="app-select" value={selected} onChange={onChange}><option value="">{placeholder}</option>{selected && !hasSelectedOption ? <option value={selected}>Loading {label.toLowerCase()}…</option> : null}{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>;
}
